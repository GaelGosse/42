...
/*   Updated: 2022/11/12 18:16:43 by ggosse           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_push_swap.h"

int	ft_is_index_lt_median(t_list **lst, int quarter)
{
	t_list	*tmp;
	int		median;
	int		size_a;
...
void	ft_pre_sort_first_half(t_list **stack_a, t_list **stack_b, int size_a)
{
	...
	size_a = ft_lstsize(tmp) ;
	median = (size_a / 2);
	while (ft_is_index_lt_median(stack_a, 0) == 1)
	{
...
void	ft_pre_sort_sec_half(t_list **stack_a, t_list **stack_b, int size_a)
{
	...
	size_a = ft_lstsize(tmp) ;
	median = (size_a / 2);
	while (ft_is_index_lt_median(stack_a, 1) == 1 && (ft_lstsize(*stack_a) > 3))
	{
		if ((*stack_a)->index < median)
		{
			ft_pb(stack_a, stack_b);
